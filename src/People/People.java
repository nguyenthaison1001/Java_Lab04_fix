...
public abstract class People {
    public String name;
    protected final Status nowStatus;
    ...
    public Integer setStatus(){
        if (nowStatus.isBefore()) return 0;
        else if (nowStatus.isCurrent()) return 1;
        else if (nowStatus.isAfter()) return -1;
            return null;
    }
    ...
    public void chooseAction(){
        ...
        while (chosenActions.isEmpty()) {
            for (Action action : allAction) {
                if (checkJob(action) & setStatus() == action.getStatus() ){
                    if (Math.random() <= 0.5) {
    ...
    public String toString(){
        ...
        if (en < 8) str += name + " is very exited. ";
        else if (en < 15) str += name + " is full of energy. ";
        else if (en < 20) str += name + "'s a bit tired. ";
        else str += "So " + name + " needs some rest. ";
        return str;
    }
